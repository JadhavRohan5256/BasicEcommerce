...
let cross = document.querySelector(".crossBar");
let box = document.querySelector("#cartView");
cross.onclick = ()=>{
...
// this function for storing all added product to the cart 
...
// this function is loading all product in index.jsp
function loadCart() {
		let storage = localStorage.getItem("cart");
		let cart = JSON.parse(storage);
		let flag = false;
		...
		$('.topCart h2').html(`Cart items(${cart.length})`);
		let products = ``;
		let totalPrice = 0;
		let discountPrice = 0;
		cart.map((item)=>{
